...
class InMemoryUserService(application: Application) extends UserServicePlugin(application) {
  private var users = Map[String, Identity](
    "play@redis.iouserpass" -> SocialUser(UserId("play@redis.io", "userpass"),
      "play", "redis", "play-redis", Some("play@redis.io"), None, AuthenticationMethod.UserPassword, None, None,
      Some(new BCryptPasswordHasher(application).hash("playredisrocks")))
  )
